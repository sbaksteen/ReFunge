...
public static class TOYS
{
    ...
    private static void CheckBounds(int dim, FungeVector vector)
    {
        for (var i = 0; i < dim; i++)
        {
            if (vector[i] <= 0)
            {
                throw new FungeReflectException(new ArgumentOutOfRangeException(nameof(vector), "Size must be positive"));
            }
        }
    }
    ...
    {
        int[] current = (highOrder ? source + size : source).ToArray(ip.Dim);
        int[] end = (highOrder ? destination : destination + size).ToArray(ip.Dim);
        int[] dest = (highOrder ? destination + size : destination).ToArray(ip.Dim);
        if (highOrder)
        {
        ...
        while (!done)
        {
            ...
            for (int i = highOrder ? ip.Dim - 1 : 0; 
    ...
    [Instruction('V')]
    public static void MoveFungeSpaceHighOrder(FungeIP ip, FungeVector source, FungeVector size, FungeVector destination)
    ...
    [Instruction('D')]
    public static FungeInt Decrement(FungeIP ip, FungeInt a) => a - 1;
    
    [Instruction('E')]
    public static FungeInt StackSum(FungeIP ip)
    {
        int sum = 0;
        while (ip.StackStack.TOSS.Size > 0)
        {
            sum += ip.PopFromStack();
        }
        return sum;
    ...
    [Instruction('I')]
    public static FungeInt Increment(FungeIP ip, FungeInt a) => a + 1;
    ...
    public static void MoveLine(FungeIP ip, FungeInt offset, int dim)
    {
        ...
        for (var i = ip.Space.MinCoords[dim]; i <= ip.Space.MaxCoords[dim]; i++)
        {
            ...
            if (temp != ' ')
            {
                line.Add((temp, newPos));
            }
        }
        foreach (var (value, newCoords) in line)
        {
            ip.Space[newCoords] = value;
        }
    }
    
    [Instruction('J')]
    public static void MoveColumn(FungeIP ip, FungeInt offset) => MoveLine(ip, offset, 1);
    
    [Instruction('O')]
    public static void MoveRow(FungeIP ip, FungeInt offset) => MoveLine(ip, offset, 0);
    ...
    [Instruction('N')]
    public static FungeInt Negate(FungeIP ip, FungeInt a) => -a;
    ...
    public static void IncrementY(FungeIP ip)
    {
        if (ip.Dim < 2)
        {
            throw new FungeReflectException(new InvalidOperationException("TOYS::Y: Dimension too low!"));
        }
    ...
    public static void IncrementZ(FungeIP ip)
    {
        if (ip.Dim < 3)
        {
            throw new FungeReflectException(new InvalidOperationException("TOYS::Z: Dimension too low!"));
        }
